...
export default function CicloDeVida({ initialCount }: CicloDeVidaProps) {
  ...
  useEffect(() => {
    ...
    const handleCounterUpdateWrapper = (event: Event) => {
      handleCounterUpdate(event as CustomEvent);
    };

    window.addEventListener("onCounterMount", handleCounterMount);
    window.addEventListener("onCounterUnmount", handleCounterUnmount);
    window.addEventListener("onCounterUpdate", handleCounterUpdateWrapper);
    ...
    return () => {
      ...
      window.removeEventListener("onCounterUpdate", handleCounterUpdateWrapper);
    };
  }, []);
      ...
      <div>
        ...
        {showCounter && (
          <>
            ...
            <div data-content>
              <Counter initialCount={initialCount} setCountCiclo={setCount} />
            </div>
          </>
        )}
      </div>
